<template>
  <div>
    ...
    >
      <div class="canvasBase" v-if="canvasBase && (isIos||isWeChat) && isShowCanvas">
        <div class="imgWrap">
          <img :src="canvasBase" alt="canvasBase">
        </div>
        <div class="hint">
          <p>{{$t('SavePic')}}</p>
          <div @click="closeSavePic"></div>
        </div>
      </div>
    </transition>
    <!-- v-if="isIos" -->
    ...
    >
      <div class="savehint" v-show="isShowSaveHint">
        <div class="hintWrap">
          <p>{{$t('DownloadImg')}}!</p>
          <div class="confirmBtn" @click="closeSavePic">{{$t('Confirm')}}</div>
        </div>
      </div>
    </transition>
  </div>
      ...
      savePic() {
        ...
        html2canvas(canvas, {scale: 1.5, logging: false, useCORS: true, height: totalH}).then(function (canvas) {
          ...
          if (!_self.isIos) {
            ...
            //_self.$alert({message:'please Long Press Save Pictures!',confirmBtnText:'Confirm'});
          }
        });
...
<style scoped>
  ...
  .groupList {
    overflow-y: auto;
    max-height: calc(100vh - .82rem - .9rem)
  }
  ...
  .hint{font-size: .32rem;color: #01aeff;margin-top: .1rem;font-weight: 600;display: flex;align-items: center;justify-content: center;flex-direction: column}
  .hint>div{width: .6rem;height: .6rem;border-radius: 50%;background: url("../../static/images/mobile/closeicon.png") no-repeat center;background-size: .4rem auto;border: 1px solid #1f7bce;margin-top: .1rem}
